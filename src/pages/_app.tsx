...
const MyApp: AppType = ({ Component, pageProps }) => {
  ...
  const { mutateAsync: getUser } = api.user.getByEmail.useMutation();
    ...
    <DynamicContextProvider
      settings={{
        ...
        eventsCallbacks: {
          onAuthSuccess: ({ user }) => {
            void (async () => {
              const userDb = await getUser({ email: user.email! });

              if (userDb && userDb.onboarded) {
                void router.push("/creator");
              } else if (userDb && !userDb.onboarded) {
